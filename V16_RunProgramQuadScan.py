...
#Real Simulation
#'''
sinograms = np.loadtxt('XdataReal.txt',delimiter=',')
opticsParams = np.loadtxt('YdataReal.txt',delimiter=',')
tag = 'Real'
nimage = opticsParams.shape[0];
sinograms = sinograms[:,np.newaxis,:,np.newaxis].reshape((nimage,16,-1,1));
#'''
##############################################################################
#Unreal Simulation
'''
...
#Test train split.
opticsScale = np.diag([1.0, 1.0, -5.0]) # this is 1/epsilon, 1/beta, 1/alpha
print(opticsScale)
n_test = testSamples;
...
if LoadModel:
    ...
    linear_model = tf.keras.models.load_model("my_model"+tag)
    ...
    linear_model.save("my_model_"+tag)
