...
def MakePhaseSpace(aX, bX, eX, phaseResolution):
    ...
    xvals = pxvals = np.linspace(-1, 1, phaseResolution)*GlobalParams['PhaseRange_Rec']
    ...
    if ComplexPhaseSpace:
        ...
        phaseSpaceX = totalPhaseSpaceX.reshape(phaseResolution, phaseResolution)
...
def MakeTrainingImages(aX, bX, eX, phaseResolution,
    ...
    for i in range (samples):
        ...
        singleSinogram, phaseSpaceX = MakeSinogram(A, B, E, phaseResolution, projections)
...
def MakeTomographyTrainingImages(aX, bX, eX, phaseResolution,
    ...
    for i in range (samples):
        ...
        phaseSpaceX = np.reshape(phaseSpaceX, (np.square(phaseResolution)), order ='C')
